...
from relimat_wrapper import relimat
from relimat.utils.get_default_model import get_relimat_model
from relimat.inference.inference_core import InferenceCore
...
def image_matting(video_state, interactive_state, mask_dropdown, erode_kernel_size, dilate_kernel_size, refine_iter, model_selection):
    ...
    relimat_processor = InferenceCore(selected_model, cfg=selected_model.cfg)
    ...
    if len(np.unique(template_mask))==1:
        template_mask[0][0]=1
    foreground, alpha = relimat(relimat_processor, following_frames, template_mask*255, r_erode=erode_kernel_size, r_dilate=dilate_kernel_size, n_warmup=refine_iter)
...
def video_matting(video_state, interactive_state, mask_dropdown, erode_kernel_size, dilate_kernel_size, model_selection):
    ...
    if len(np.unique(template_mask))==1:
        template_mask[0][0]=1
    foreground, alpha = relimat(relimat_processor, following_frames, template_mask*255, r_erode=erode_kernel_size, r_dilate=dilate_kernel_size)
...
# initialize relimat - lazy loading
...
# Model URLs
model_urls = {
    "relimat.pth": "https://github.com/pq-yang/ReliMat/releases/download/v1.0.0/relimat.pth",
    "relimat.pth": "https://github.com/pq-yang/relimat/releases/download/v1.0.0/relimat.pth"
}

# Model paths - download models using load_file_from_url
model_paths = {
    "relimat.pth": load_file_from_url(model_urls["relimat.pth"], checkpoint_folder),
    "relimat.pth": load_file_from_url(model_urls["relimat.pth"], checkpoint_folder)
}
...
def load_model(display_name):
    ...
    model = get_relimat_model(ckpt_path, args.device)
...
# Order: ReliMat first, then ReliMat
available_models = []
# Check ReliMat first
if "ReliMat" in model_display_to_file:
    file_name = model_display_to_file["ReliMat"]
    if file_name in model_paths and os.path.exists(model_paths[file_name]):
        available_models.append("ReliMat")
# Then check ReliMat
if "ReliMat" in model_display_to_file:
    file_name = model_display_to_file["ReliMat"]
    if file_name in model_paths and os.path.exists(model_paths[file_name]):
...
default_model = "ReliMat" if "ReliMat" in available_models else available_models[0]

# download test samples
test_sample_path = os.path.join('.', "test_sample/")
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-0-1080p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-1-1080p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-2-720p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-3-720p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-4-720p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-5-720p.mp4', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-0.jpg', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-1.jpg', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-2.jpg', test_sample_path)
load_file_from_url('https://github.com/pq-yang/relimat/releases/download/media/test-sample-3.jpg', test_sample_path)

# download assets
assets_path = os.path.join('.', "assets/")
load_file_from_url('https://github.com/pq-yang/ReliMat/releases/download/media/tutorial_single_target.mp4', assets_path)
load_file_from_url('https://github.com/pq-yang/ReliMat/releases/download/media/tutorial_multi_targets.mp4', assets_path)

# documents
title = r"""<div class="multi-layer" align="center"><span>ReliMat Series</span></div>
"""
description = r"""
<b>Official Gradio demo</b> for <a href='https://github.com/pq-yang/relimat' target='_blank'><b>ReliMat</b></a> and <a href='https://github.com/pq-yang/ReliMat' target='_blank'><b>ReliMat</b></a>.<br>
🔥 ReliMat series provide practical human video matting framework supporting target assignment.<br>
🧐 <b>We use <u>ReliMat</u> as the default model. You can also choose <u>ReliMat</u> in "Model Selection".</b><br>
...
launching the demo locally following the <a href='https://github.com/pq-yang/relimat?tab=readme-ov-file#-interactive-demo' target='_blank'>GitHub instructions</a>.</b>
"""
article = r"""<h3>
<b>If our projects are helpful, please help to 🌟 the Github Repo for <a href='https://github.com/pq-yang/relimat' target='_blank'>ReliMat</a> and <a href='https://github.com/pq-yang/ReliMat' target='_blank'>ReliMat</a>. Thanks!</b></h3>
...
```bibtex
@InProceedings{yang2026relimat,
      title     = {{ReliMat}: Scaling Video Matting via a Learned Quality Evaluator},
      author    = {Yang, Peiqing and Zhou, Shangchen and Hao, Kai and Tao, Qingyi},
      booktitle = {CVPR},
      year      = {2026}
}

@InProceedings{yang2025relimat,
     title     = {{ReliMat}: Stable Video Matting with Consistent Memory Propagation},
...
📝 **License**
<br>
This project is licensed under <a rel="license" href="https://github.com/pq-yang/ReliMat/blob/main/LICENSE">S-Lab License 1.0</a>. 
