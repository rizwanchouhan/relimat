...
from relimat.utils.download_util import load_file_from_url
from relimat.utils.inference_utils import gen_dilate, gen_erosion, read_frame_from_videos
from relimat.inference.inference_core import InferenceCore
from relimat.utils.get_default_model import get_relimat_model
from relimat.utils.device import get_default_device, safe_autocast_decorator

import warnings
warnings.filterwarnings("ignore")

app = typer.Typer(help="relimat video matting inference")
...
def run_inference(input_path, mask_path, output_path, ckpt_path, n_warmup=10, r_erode=10,
    ...
    pretrain_model_url = "https://github.com/pq-yang/relimat/releases/download/v1.0.0/relimat.pth"
    ckpt_path = load_file_from_url(pretrain_model_url, 'pretrained_models')

    # load ReliMat model
    relimat = get_relimat_model(ckpt_path, device)

    # init inference processor
    processor = InferenceCore(relimat, cfg=relimat.cfg)
...
def main(
    ...
    ckpt_path: Annotated[str, typer.Option("-c", "--ckpt-path",
        help="Path of the relimat model.")] = "pretrained_models/relimat.pth",
    ...
    """Run relimat video matting inference."""
    run_inference(
